...
def test_eac_single_buy_order_single_sell_order():
    ...
    assert(result["prices_rounded"][window]["POWER"] == 60.0)
...
def test_eac_multiple_buys_single_sell():
    ...
    assert(result["prices_rounded"][window]["POWER"] == 60.0), "Clearing price incorrect"
    # Verify welfare calculation
    welfare_buy = sum(b.price * b.quantity * result["x_b"][b.orderID] for b in buy_orders)
...
def test_eac_looped_baskets():
    ...
    assert(result["prices_rounded"][window]["POWER"] == 65.0)
    # Verify welfare calculation
    welfare_buy = sum(b.price * b.quantity * result["x_b"][b.orderID]
                        for b in buy_orders)
    welfare_sell = sum(s.price * s.quantity * result["x_s"][s.orderID]
...
def test_eac_overholding():
    ...
    assert(result["x_b"]["BUY_1"] == 1.0)
    assert(result["x_s"]["SELL_1"] == 1.0)
    assert(result["prices_rounded"][window]["POWER"] == 40.0)
    # Verify welfare calculation
    welfare_buy = sum(b.price * b.quantity * result["x_b"][b.orderID] for b in buy_orders)
    welfare_sell = sum(s.price * s.quantity * result["x_s"][s.orderID] 
                      for s in sell_orders)
...
def test_eac_child_rejection_if_parent_not_accepted():
    ...
    assert(result["prices_rounded"][window]["POWER"] == 0.0)  # Price should not change
...
def test_mcp_multi_product_not_max_sell():
    ...
    assert result["prices_rounded"][window]["P1"] >= 10.00
    assert result["prices_rounded"][window]["P2"] >= 0

def test_rejecting_sell_orders():
    products = ["POWER"]
    ...
    sell_orders = [
        create_sell_order("SELL_1", "POWER", 0, 0.0, basket_id=1, order_type="parent", min_acceptance_ratio=1.0),
        create_sell_order("SELL_2", "POWER", 1, 101.0, basket_id=2, order_type="child", min_acceptance_ratio=1.0)
    ]
    
    baskets = [
        create_basket("BASKET_1", "UNIT_1"),
        create_basket("BASKET_2", "UNIT_1")
    ]
    ...
    window = ("2025-01-01", "2025-01-02")
    assert(result.get("final"))
    assert(result["x_s"]["SELL_1"] == 0.0)  # Sell order rejected due to min_acceptance_ratio requirement
    assert(result["x_s"]["SELL_2"] == 0.0)  # Sell order rejected due to high price
    assert(result["prices_rounded"][window]["POWER"] == 0.0)
    # Verify welfare calculation
    welfare_buy = sum(b.price * b.quantity * result["x_b"][b.orderID] for b in buy_orders)
    welfare_sell = sum(s.price * s.quantity * result["x_s"][s.orderID] 
                      for s in sell_orders)
