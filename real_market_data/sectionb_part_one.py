...
if __name__ == "__main__":
    ...
    beta  = [1]
    revenue_results = defaultdict(list)
    ...
    for auction_index, auction_id in enumerate(auction_ids):
        ...
        for a in alpha:
            for b in beta:
                ...
                # Add cumulative revenue for this beta value
                if len(revenue_results[a]) == 0:
                    revenue_results[a].append(auction_revenue)
                else:
                    revenue_results[a].append(revenue_results[a][-1] + auction_revenue)
    ...
    for a in alpha:
        plt.plot(range(len(revenue_results[a])), revenue_results[a], label=f"Alpha={a}")
    plt.xlabel("Auction Index")
    plt.ylabel("Cumulative Revenue (£)")
    plt.title(f"Cumulative Revenue vs Auction Index for varying Alpha (Beta={beta[0]})")
    plt.legend()
    plt.show()